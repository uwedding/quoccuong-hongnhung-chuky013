...
const GOOGLE_SCRIPT_URL_THAM_DU =
  "https://script.google.com/macros/s/AKfycby1FDTGbOqUFJtV15C6IfbxVhkNAWISm3wFWaFRQhbLO5bNt8y7aum9bs28Ch_aOg0W/exec";
document
  ...
  .addEventListener("submit", function (e) {
    ...
    let selected6 = formData.getAll("form_item6");
    let selected8 = formData.getAll("form_item8");

    const data = {
      name: "'" + form.name.value,
      form_item6: "'" + selected6.join(", "), // Sẽ đến chứ
      form_item23: "'" + form.form_item19.value, // Số khách
      form_item8: "'" + selected8.join(", "), // Tham dự tiệc nào
      form_item13: "'" + selected.join(", "), // Khách của ai
      form_item12: "'" + form.form_item12.value, // Ai thấy lời chúc
...
// ...existing code...
function setCloseIcon(btn) {
  btn.innerHTML = `
    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" aria-hidden="true" role="img">
  ...
  // Kiểu để đảm bảo kích thước hiển thị và tương thích mobile
...
const applyOrObserve = () => {
  const btn =
    document.getElementById("closePopup") ||
    document.querySelector(".ClassClosePopup");
  if (btn) {
    setCloseIcon(btn);
    return;
  }
  ...
  setTimeout(() => mo.disconnect(), 5000); // ngắt sau 5s nếu chưa thấy
};

if (document.readyState === "loading")
  document.addEventListener("DOMContentLoaded", applyOrObserve);
else applyOrObserve();
